...
--
-- Hôte : 127.0.0.1:3307
-- Généré le : mar. 17 fév. 2026 à 12:39
...
--
-- Doublure de structure pour la vue `monitoring_view`
-- (Voir ci-dessous la vue réelle)
--
CREATE TABLE `monitoring_view` (
...
--
-- Structure de la vue `monitoring_view`
--
DROP TABLE IF EXISTS `monitoring_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `monitoring_view`  AS SELECT `t`.`statut` AS `statut`, count(0) AS `COUNT(*)` FROM `tickets` AS `t` GROUP BY `t`.`statut` ;
